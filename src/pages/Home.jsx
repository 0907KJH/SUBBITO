...
export default function Home() {
  ...
  const handleCalculate = () => {
    ...
    if (!config.setup_primario) {
      setErrors(prev => [...prev, "❌ Seleziona un setup principale prima di calcolare"]);
      return;
    }
    
    const numSubs = parseInt(config.numero_subwoofer);
    if (isNaN(numSubs) || numSubs < 1) {
      setErrors(prev => [...prev, "❌ Inserisci un numero valido di subwoofer (minimo 1)"]);
      return;
    }
    ...
    if (config.setup_primario === 'endfire') {
      ...
      if (isNaN(numLinee) || numLinee < 2) {
        setErrors(prev => [...prev, "❌ Endfire: numero di linee deve essere almeno 2"]);
        return;
      }
      ...
      if (numSubs % numLinee !== 0) {
        ...
        setErrors(prev => [...prev, 
          `❌ Endfire: ${numSubs} subwoofer non sono divisibili per ${numLinee} linee`,
    ...
    if (config.setup_primario === 'gradient') {
      if (numSubs % 2 !== 0) {
        setErrors(prev => [...prev, 
          `❌ Gradient: richiede un numero PARI di subwoofer`,
          `💡 Hai ${numSubs} sub. Usa ${numSubs - 1} o ${numSubs + 1} sub.`
        ]);
        return;
      }
      ...
      if (isNaN(distFisica) || distFisica <= 0 || config.distanza_fisica_gradient === "") {
        setErrors(prev => [...prev, "❌ Gradient: specifica la distanza fisica Front-Rear in centimetri"]);
        return;
      }
      ...
      if (distFisica > maxDistCm) {
        setErrors(prev => [...prev, 
          `❌ Gradient: distanza fisica troppo grande`,
          `💡 Massimo consentito: ${maxDistCm} cm (Î»/2 @ ${config.frequenza_crossover} Hz)`,
          `💡 Hai inserito: ${distFisica} cm`
        ]);
        return;
      }
    }
    ...
    if (config.setup_primario === 'l_r') {
      if (numSubs % 2 !== 0) {
        setErrors(prev => [...prev,
          `❌ L - R: richiede un numero PARI di subwoofer`,
          `💡 Hai ${numSubs} sub. Usa ${numSubs - 1} o ${numSubs + 1} sub.`
        ]);
        return;
      }

      // Validazioni specifiche quando è selezionato un setup secondario in L-R
      const sec = config.setup_secondario || 'nessuno';
      // ENDFIRE secondario: almeno 2 per lato (>=4 totali) e freq valida
      if (sec === 'endfire') {
        if (numSubs < 4) {
          setErrors(prev => [...prev,
            `❌ L - R + Endfire: servono almeno 4 sub totali (min 2 per lato)`,
            `💡 Hai ${numSubs} sub. Aumenta a 4 o più.`
          ]);
          return;
        }
        ...
        if (isNaN(fTarget) || fTarget < 20 || fTarget > 200) {
          setErrors(prev => [...prev,
            `❌ Endfire secondario: specifica la Frequenza Target tra 20 e 200 Hz`,
            `💡 Valore attuale: ${config.frequenza_target_cancellazione || 'n.d.'}`
          ]);
          return;
        }
      }
      ...
      if (sec === 'gradient') {
        if (numSubs !== 4) {
          setErrors(prev => [...prev,
            `❌ L - R + Gradient: supportiamo SOLO 4 sub totali (1 coppia per lato)`,
            `💡 Hai ${numSubs} sub. Imposta esattamente 4 sub per usare Gradient come secondario in Lâ€‘R.`
          ]);
          return;
        }
        const distFisica = parseFloat(config.distanza_fisica_gradient);
        if (isNaN(distFisica) || distFisica <= 0 || config.distanza_fisica_gradient === "") {
          setErrors(prev => [...prev, `❌ Gradient secondario: specifica la distanza fisica F/R in centimetri`]);
          return;
        }
        const wavelength = 343 / (config.frequenza_crossover || 80);
        const maxDistCm = Math.round((wavelength / 2) * 100);
        if (distFisica > maxDistCm) {
          setErrors(prev => [...prev,
            `❌ Gradient secondario: distanza fisica troppo grande` ,
            `💡 Massimo consentito: ${maxDistCm} cm (Î»/2 @ ${config.frequenza_crossover} Hz)`,
            `💡 Hai inserito: ${distFisica} cm`
          ]);
          return;
      ...
      // STACK CARDIOID secondario: N totale pari, 4 â‰¤ N â‰¤ 6, max 3 per lato, profondità sub richiesta
      if (sec === 'stack_cardioid') {
        if (numSubs < 4) {
          setErrors(prev => [...prev,
            `❌ L - R + Stack Cardioid: servono almeno 4 sub totali (2 per lato)`,
            `💡 Hai ${numSubs} sub. Aumenta a 4 o più.`
          ]);
          return;
        }
        if (numSubs > 6) {
          setErrors(prev => [...prev,
            `❌ L - R + Stack Cardioid: massimo 6 sub totali (max 3 per lato)`,
            `💡 Hai ${numSubs} sub. Riduci a 6 o meno.`
          ]);
          return;
        }
        ...
        if (perLato > 3) {
          setErrors(prev => [...prev,
            `❌ L - R + Stack Cardioid: massimo 3 sub per lato`,
            `💡 Hai ${perLato} per lato. Riduci il totale a 6 o meno.`
          ]);
          return;
        }
        ...
        if (isNaN(profSub) || profSub <= 0 || config.profondita_sub_cardioid === "") {
          setErrors(prev => [...prev,
            `❌ Stack Cardioid secondario: specifica la profondità fisica del sub in centimetri`,
            `💡 Esempio: per un sub 18" usa ~60 cm`
          ]);
          return;
        }
      }
  ...
  const setField = (field) => (valOrEvent) => {
    ...
    // Se è gradi_arc, aggiorna anche sessionStorage e notifica Overview
    if (field === 'gradi_arc') {
      const stored = sessionStorage.getItem('subbito_results');
      if (stored) {
        const data = JSON.parse(stored);
        ...
        <Card className={`mb-6 ${bgCard} ${borderCard}`}>
          <CardContent className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Numero di linee è stato spostato all'interno del box Endfire */}

              {config.setup_primario === 'gradient' && (
                <>
                  <div className="space-y-3 p-4 border border-green-500/30 rounded-lg bg-green-500/5">
                    {/* Setup Primario INSCRITTO nel box quando Gradient è attivo */}
                    <div className="space-y-2">
                      <Label htmlFor="setup_primario" className="text-green-400">Setup Primario</Label>
                      <Select 
                        value={config.setup_primario} 
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative pt-2 pb-1">
                        ...
                        <div 
                          ...
                          style={{ 
                            left: `${(25 / 50) * 100}%`, // Î»/4 è al 50% tra 0 e Î»/2
              ...
              {config.setup_primario === 'arc' && (
                <>
                  <div className="space-y-3 p-4 border border-green-500/30 rounded-lg bg-green-500/5">
                    {/* Setup Primario INSCRITTO nel box quando Arc è attivo */}
                    <div className="space-y-2">
                      <Label htmlFor="setup_primario" className="text-green-400">Setup Primario</Label>
                      <Select 
                        value={config.setup_primario} 
              ...
              {config.setup_primario === 'stack_cardioid' && (
                <div className="space-y-3 p-4 border border-green-500/30 rounded-lg bg-green-500/5">
                  {/* Setup Primario INSCRITTO nel box quando Stack Cardioid è attivo */}
                  <div className="space-y-2">
                    <Label htmlFor="setup_primario" className="text-green-400">Setup Primario</Label>
                    <Select 
                      value={config.setup_primario} 
              ...
              {/* Select Setup Secondario posizionato dopo il box primario quando NON è Arc */}
              {/* Select Setup Secondario (generico) - NON mostrare quando Primario è L-R */}
              ...
              {config.setup_primario === 'l_r' && (
                <>
                  {/* Se nessuno è selezionato, mostra solo il select (fuori dal box) */}
            ...
            {showSuccess && (
              <Alert className="mt-4 border-green-500 bg-green-50 dark:bg-green-900/20">
                ...
                <AlertDescription className="text-green-700 dark:text-green-300">
                  ✅ Calcolo completato! Ora puoi navigare a Overview, Predizione o Report.
                </AlertDescription>
              </Alert>
            )}
          </CardContent>
          ...
          onSave={async (configName) => {
            try {
              ...
              if (res.success) {
                ...
                if (!res.canceled) alert(`❌ Errore: ${res.error || 'Operazione annullata'}`);
              }
            } catch (e) {
              alert(`❌ Errore inatteso: ${e?.message || e}`);
            }
          }}
        />
      </div>
